...
async function addRelType({ name, directed = true }) {
  ...
  await write(
    `MERGE (t:_RelType {name: $name})
     ON CREATE SET t.directed = $directed, t.builtin = false
     RETURN t`,
    { name: upper, directed: directed !== false },
  );
  await loadRegistry();
...
module.exports = {
  ...
  addEntityType, addRelType, deleteEntityType, deleteRelType, validName,
};
